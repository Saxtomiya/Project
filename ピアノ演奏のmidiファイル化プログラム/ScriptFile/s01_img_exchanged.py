...
def make_img(num, yy, point):# 画像をクリッピング、射影変換する関数。
    ...
    # 画像の読み込み
    img_src1 = cv2.imread("data/img/01image/{0:03d}.jpg".format(num), 1)
    img_cri01 = img_src1[yy+5:yy+105, 0:1280]
    cri_gray = cv2.cvtColor(img_cri01, cv2.COLOR_RGB2GRAY)
    cri_edge = cv2.Canny(cri_gray, 100, 100, True)
    perspective1 = np.float32([[0, 0], [1080, 0], [0, 100], [1080, 100]])
    perspective2 = np.float32([[0, 0], [1036, 0], [5, 100], [1030, 100]])
    ...
    result = img_psp[0:100, 100:1080]
    cv2.imwrite("data/img/02crip/{0:03d}.jpg".format(num), result)
    #02cripに変換した画像を保存
